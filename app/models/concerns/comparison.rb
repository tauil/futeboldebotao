module Comparison
  ...
  included do
    scope :goals_pro_for_player1_vs_player2, lambda { |player1_id, player2_id, year|
      ...
      arel_home_score_query = arel_home_score_query.where(matches_by_occurred_at_arel(year))
      ...
      arel_visitor_score_query = matches.where(matches[:home_player_id].eq(player2_id).and(matches[:visitor_player_id].eq(player1_id)))
      arel_visitor_score_query = arel_visitor_score_query.where(matches_by_occurred_at_arel(year))
    ...
    scope :unscoped_won, lambda { |player_id|
      where(Match.arel_table[:winner_id].eq(player_id))
    }

    scope :unscoped_draw, lambda { where(Match.arel_table[:winner_id].eq(nil)) }
  end
end
